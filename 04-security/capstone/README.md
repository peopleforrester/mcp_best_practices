...
2. **Policy gateway (outermost middleware).** `PolicyMiddleware` evaluates each call against the
   per-client token-bucket rate limit, the allowlist, OPA-style rules, and the consent gate, and
   writes a SIEM-ready audit record. A destructive tool without consent is denied with a `ToolError`
   before it runs; an over-limit client is throttled the same way. (MCP02/MCP05/MCP08/MCP09.)
...
allowed with it, an over-limit client is rate-limited with the denial audited, an injection pattern in
a tool result reaches the finding sink (and is logged by default), and a secret in a tool result is
redacted before return, including inside nested structured content. Real Ed25519 signatures, no
